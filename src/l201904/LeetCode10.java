...
public class LeetCode10 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(l.isMatch2("aa", "a*"));
    ...
    public boolean match(char[] s, char[] p, int index1, int index2) {
        if (index2 >= p.length) {
            return index1 >= s.length;
        }
        boolean m1 = index1 < s.length && (s[index1] == p[index2] || p[index2] == '.');
        if (index2 < p.length - 1 && p[index2 + 1] == '*') {
            return match(s, p, index1, index2 + 2) || (m1 && match(s, p, index1 + 1, index2));
        } else {
            return m1 && match(s, p, index1 + 1, index2 + 1);
        }
    }
}
